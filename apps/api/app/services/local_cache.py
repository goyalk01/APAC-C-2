# MOVED from swarmguard-ai/swarm_box/local_cache.py — logic unchanged.
# Structural change only: DB_PATH is now sourced from Settings (env-driven)
...
def get_all_threats(limit: int | None = None, offset: int = 0) -> list[dict]:
    """Return cached threats, most recent first.

    `limit`/`offset` are new optional params (structural pagination addition,
    see ARCHITECTURE_PLAN.md §2). When limit is None, behavior is identical
    to the original unbounded query.
    """
    conn = get_connection()
    if limit is not None:
        rows = conn.execute(
            "SELECT * FROM threat_events ORDER BY id DESC LIMIT ? OFFSET ?",
            (limit, offset),
        ).fetchall()
    else:
...
def count_threats() -> int:
    conn = get_connection()
    row = conn.execute("SELECT COUNT(*) as c FROM threat_events").fetchone()
    conn.close()
    return row["c"]
...
def db_healthy() -> bool:
    """Used by /readyz — checks SQLite is reachable."""
    try:
        conn = get_connection()
