...
|---|---|---|---|
| **Edge Node Simulator** | **WORKING** | `edge_nodes/edge_node.py:8` (generates signed payload) | ✅ Jitter added — confidence ±5%, delay +1-3s, GPS ±0.001° |
| **Cryptographic Signing** | **WORKING** | `edge_nodes/crypto_signer.py:6` (Ed25519 sign) | ✅ End-to-end: keys pre-shared via `node_keys.json`, verified in backend API |
...
| **Mesh Routing** | **PARTIAL** | `mesh_network/mesh_relay.py` (broadcast loop) | Centralized WebSocket hub, not a mesh. Prototype uses WebSocket relay. |
...
| **Cloud Run Sync API** | **STUBBED** | `cloud_layer/sync_api.py` | Loops payloads, logs to console, returns success. No BigQuery. |
| **BigQuery Integration** | **STUBBED** | `cloud_layer/bigquery_schema.sql` | Schema file exists, zero Python code writes to it. |
...
*   **The JSON Payload Schema:** `edge_node.py` generates specific keys (`gps.lat`, `confidence`, `alert_type`, `signature`), which API and Next.js extract. Do not rename or change these keys.
